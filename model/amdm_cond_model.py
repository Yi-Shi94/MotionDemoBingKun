...
class GaussianDiffusion(nn.Module):
    ...
    def __init__(
        ...
        self.cond_size = config["model_hyperparam"].get("cond_size", 768)
        self.cond_mask_prob = config["model_hyperparam"].get("cond_mask_prob", 0.1)
        self.cond_mask_scale = config["model_hyperparam"].get("cond_mask_scale", 2.5)
        ...
        self.cond_mlp = nn.Sequential(
            nn.Linear(self.cond_size, self.time_emb_dim),
            Activation.SiLU(),
    ...
    def sample_ddpm(self, last_x, extra_info):
        ...
        cond_latent = self.cond_mlp(extra_info['cond'])
        uncond_latent = torch.zeros_like(cond_latent) if 'neg_cond' not in extra_info else \
                        self.cond_mlp(extra_info['neg_cond'])
        
        for t in range(self.T - 1, -1, -1):
            ts = torch.tensor([t], device = last_x.device).repeat(last_x.shape[0])
            time_emb = self.time_mlp(ts)
           
            cond_pred = self.model(last_x, x, time_emb + cond_latent).detach()
            uncond_pred = self.model(last_x, x, time_emb + uncond_latent).detach()
            pred = uncond_pred + self.cond_mask_scale * (cond_pred - uncond_pred)
    ...
    def sample_ddim(self, last_x, num_steps, eta=0.0, extra_info=None):
        ...
        if self.use_cond:
            cond = extra_info['cond']
            uncond = extra_info['neg_cond'] if 'neg_cond' in extra_info else None
        ...
        for t in range(len(timesteps)-1, -1, -1):
            ...
            cond_pred = self.model(last_x, x, ts, cond).detach()
            uncond_pred = self.model(last_x, x, ts, uncond).detach()
            pred = uncond_pred + self.cond_mask_scale * (cond_pred - uncond_pred)
    ...
    def forward(self, cur_x, next_x, extra_info):
        ...
        cond = extra_info['cond']
        cond_latent = self.cond_mlp(cond)
        mask = torch.bernoulli(torch.ones(bs, device=device) * self.cond_mask_prob).view(bs, 1)  # 1-> use null_cond, 0-> use real cond
        cond_latent = cond_latent * (1. - mask)

        noise = torch.randn_like(next_x)
        perturbed_x = self.perturb_x(next_x, ts, noise)
        
        latent = time_emb + cond_latent
