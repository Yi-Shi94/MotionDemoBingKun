...
class AMDMCONDTrainer(trainer_base.BaseTrainer):
    NAME = 'AMDMCOND'
    def __init__(self, config, dataset, device):
        super(AMDMCONDTrainer, self).__init__(config, dataset, device)
    ...
    def compute_teacher_loss(self, model, sampled_frames, extra_info):
        ...
        for k in extra_info:
            extra_info[k] = extra_info[k][:,st_index,:]
    ...
    def compute_student_loss(self, model, sampled_frames, extra_info):
        ...
        for st_index in range(self.num_rollout - 1):
            cur_extra_info = {}
            for k in extra_info:
                cur_extra_info[k] = extra_info[k][:,st_index,:]
            next_index = st_index + 1

            ground_truth = sampled_frames[:,next_index,:]
            loss_diff = model.compute_loss(last_frame, ground_truth, cur_extra_info)
            
            with torch.no_grad():
                x = model.eval_step(last_frame, cur_extra_info)
    ...
    def train_loop(self, ep, model):
        ...
        model.train()
        for i, (frames, conds) in enumerate(self.train_dataloader):
            ...
            conds = conds.to(self.device).float()

            extra_info = {'cond':conds}
